...
function thresholdFreedmanDiaconis(values, min5, max6) {
  const c4 = count(values), d = quantile2(values, 0.75) - quantile2(values, 0.25);
  return c4 && d ? Math.ceil((max6 - min5) / (2 * d * Math.pow(c4, -1 / 3))) : 1;
}
...
function thresholdScott(values, min5, max6) {
  ...
  return c4 && d ? Math.ceil((max6 - min5) * Math.cbrt(c4) / (3.49 * d)) : 1;
}
  ...
  *neighbors(i) {
  ...
  _project(x0, y0, vx, vy) {
    ...
    if (vy < 0) {
      if (y0 <= this.ymin)
        return null;
      if ((c4 = (this.ymin - y0) / vy) < t)
        y4 = this.ymin, x4 = x0 + (t = c4) * vx;
    } else if (vy > 0) {
      if (y0 >= this.ymax)
        return null;
      if ((c4 = (this.ymax - y0) / vy) < t)
        y4 = this.ymax, x4 = x0 + (t = c4) * vx;
    }
    if (vx > 0) {
      if (x0 >= this.xmax)
        return null;
      if ((c4 = (this.xmax - x0) / vx) < t)
        x4 = this.xmax, y4 = y0 + (t = c4) * vy;
    } else if (vx < 0) {
      if (x0 <= this.xmin)
        return null;
      if ((c4 = (this.xmin - x0) / vx) < t)
        x4 = this.xmin, y4 = y0 + (t = c4) * vy;
    }
    return [x4, y4];
  }
  _edgecode(x4, y4) {
...
function collinear2(d) {
  ...
  for (let i = 0; i < triangles.length; i += 3) {
    const a4 = 2 * triangles[i], b = 2 * triangles[i + 1], c4 = 2 * triangles[i + 2], cross3 = (coords[c4] - coords[a4]) * (coords[b + 1] - coords[a4 + 1]) - (coords[b] - coords[a4]) * (coords[c4 + 1] - coords[a4 + 1]);
    if (cross3 > 1e-10)
      return false;
  }
  return true;
...
function link_default(links) {
  var id = index3, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count4, bias, random, iterations = 1;
  if (links == null)
    links = [];
...
function cluster_default() {
  ...
  function cluster(root4) {
    ...
    var left = leafLeft(root4), right = leafRight(root4), x0 = left.x - separation(left, right) / 2, x1 = right.x + separation(right, left) / 2;
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon5))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon5 && (padRadius ? +padRadius.apply(this, arguments) : sqrt2(r0 * r0 + r1 * r1)), rc = min4(abs3(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t02, t12;
      if (rp > epsilon5) {
...
function pie_default() {
  var value = identity_default3, sortValues = descending_default, sort3 = null, startAngle = constant_default10(0), endAngle = constant_default10(tau4), padAngle = constant_default10(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum5 = 0, index4 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau4, Math.max(-tau4, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
