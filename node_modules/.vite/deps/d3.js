...
} from "./chunk-6OVF5HL2.js";
import {
  InternMap,
  InternSet
} from "./chunk-C5SBQORP.js";
...
function bin() {
  ...
  function histogram(data) {
    ...
    for (i = 0; i <= m3; ++i) {
      ...
      bin3.x1 = i < m3 ? tz[i] : x1;
    }
    if (isFinite(step)) {
      if (step > 0) {
        for (i = 0; i < n; ++i) {
          if ((x4 = values[i]) != null && x0 <= x4 && x4 <= x1) {
            bins[Math.min(m3, Math.floor((x4 - x0) / step))].push(data[i]);
          }
        }
      } else if (step < 0) {
        for (i = 0; i < n; ++i) {
          if ((x4 = values[i]) != null && x0 <= x4 && x4 <= x1) {
            const j = Math.floor((x0 - x4) * step);
...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format2 = tickFormat2 == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default2 : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range4 = scale2.range(), range0 = +range4[0] + offset, range1 = +range4[range4.length - 1] + offset, position = (scale2.bandwidth ? center : number2)(scale2.copy(), offset), selection4 = context.selection ? context.selection() : context, path2 = selection4.selectAll(".domain").data([null]), tick = selection4.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
    ...
    dx = k2 ? padAngle : tau2 / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups3[i] = { index: i, startAngle: x0, endAngle: x4, value: groupSums[i] };
        } else {
...
function link_default(links) {
  var id2 = index3, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count4, bias, random, iterations = 1;
  if (links == null)
    links = [];
  ...
  function initialize() {
    ...
    for (i = 0, count4 = new Array(n); i < m3; ++i) {
      ...
      if (typeof link2.source !== "object")
        link2.source = find3(nodeById, link2.source);
      if (typeof link2.target !== "object")
        link2.target = find3(nodeById, link2.target);
...
function locale_default(locale2) {
  var group3 = locale2.grouping === void 0 || locale2.thousands === void 0 ? identity_default3 : formatGroup_default(map5.call(locale2.grouping, Number), locale2.thousands + ""), currencyPrefix = locale2.currency === void 0 ? "" : locale2.currency[0] + "", currencySuffix = locale2.currency === void 0 ? "" : locale2.currency[1] + "", decimal = locale2.decimal === void 0 ? "." : locale2.decimal + "", numerals = locale2.numerals === void 0 ? identity_default3 : formatNumerals_default(map5.call(locale2.numerals, String)), percent = locale2.percent === void 0 ? "%" : locale2.percent + "", minus = locale2.minus === void 0 ? "−" : locale2.minus + "", nan = locale2.nan === void 0 ? "NaN" : locale2.nan + "";
...
function cluster_default() {
  ...
  function cluster(root5) {
    ...
    var left2 = leafLeft(root5), right2 = leafRight(root5), x0 = left2.x - separation(left2, right2) / 2, x1 = right2.x + separation(right2, left2) / 2;
...
function cornerTangents(x0, y0, x1, y1, r1, rc, cw) {
  var x01 = x0 - x1, y01 = y0 - y1, lo = (cw ? rc : -rc) / sqrt2(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x0 + ox, y11 = y0 + oy, x10 = x1 + ox, y10 = y1 + oy, x00 = (x11 + x10) / 2, y00 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D2 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt2(max5(0, r * r * d2 - D2 * D2)), cx0 = (D2 * dy - dx * d) / d2, cy0 = (-D2 * dx - dy * d) / d2, cx1 = (D2 * dy + dx * d) / d2, cy1 = (-D2 * dx + dy * d) / d2, dx0 = cx0 - x00, dy0 = cy0 - y00, dx1 = cx1 - x00, dy1 = cy1 - y00;
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon6))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon6 && (padRadius ? +padRadius.apply(this, arguments) : sqrt2(r0 * r0 + r1 * r1)), rc = min4(abs3(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t02, t12;
      if (rp > epsilon6) {
...
function area_default4(x0, y0, y1) {
  var x1 = null, defined = constant_default10(true), context = null, curve = linear_default, output = null, path2 = withPath(area);
  x0 = typeof x0 === "function" ? x0 : x0 === void 0 ? x3 : constant_default10(+x0);
  y0 = typeof y0 === "function" ? y0 : y0 === void 0 ? constant_default10(0) : constant_default10(+y0);
  y1 = typeof y1 === "function" ? y1 : y1 === void 0 ? y3 : constant_default10(+y1);
  function area(data) {
  ...
  area.x = function(_) {
    return arguments.length ? (x0 = typeof _ === "function" ? _ : constant_default10(+_), x1 = null, area) : x0;
  };
  area.x0 = function(_) {
    return arguments.length ? (x0 = typeof _ === "function" ? _ : constant_default10(+_), area) : x0;
  };
  area.x1 = function(_) {
    return arguments.length ? (x1 = _ == null ? null : typeof _ === "function" ? _ : constant_default10(+_), area) : x1;
  };
  area.y = function(_) {
    return arguments.length ? (y0 = typeof _ === "function" ? _ : constant_default10(+_), y1 = null, area) : y0;
  };
  area.y0 = function(_) {
    return arguments.length ? (y0 = typeof _ === "function" ? _ : constant_default10(+_), area) : y0;
  };
  area.y1 = function(_) {
    return arguments.length ? (y1 = _ == null ? null : typeof _ === "function" ? _ : constant_default10(+_), area) : y1;
  };
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort3 = null, startAngle = constant_default10(0), endAngle = constant_default10(tau5), padAngle = constant_default10(0);
  function pie(data) {
...
function link(curve) {
  ...
  link2.x = function(_) {
    return arguments.length ? (x4 = typeof _ === "function" ? _ : constant_default10(+_), link2) : x4;
  };
  link2.y = function(_) {
    return arguments.length ? (y4 = typeof _ === "function" ? _ : constant_default10(+_), link2) : y4;
  };
...
var asterisk_default = {
  draw(context, size) {
    const r = sqrt2(size + min4(size / 28, 0.75)) * 0.59436;
    const t = r / 2;
    const u4 = t * sqrt3;
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var plus_default = {
  draw(context, size) {
    const r = sqrt2(size - min4(size / 7, 2)) * 0.87559;
    context.moveTo(-r, 0);
    context.lineTo(r, 0);
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var times_default = {
  draw(context, size) {
    const r = sqrt2(size - min4(size / 6, 1.7)) * 0.6189;
    context.moveTo(-r, -r);
    context.lineTo(r, r);
    context.moveTo(-r, r);
    context.lineTo(r, -r);
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection4, k2, p, event) {
    zoom.transform(selection4, function() {
      var e = extent3.apply(this, arguments), t02 = this.__zoom, p0 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t02.invert(p0), k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
  ...
  zoom.translateTo = function(selection4, x4, y4, p, event) {
    zoom.transform(selection4, function() {
      var e = extent3.apply(this, arguments), t = this.__zoom, p0 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p;
  ...
  function dblclicked(event, ...args) {
    if (!filter6.apply(this, arguments))
      return;
    var t02 = this.__zoom, p0 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t02.invert(p0), k1 = t02.k * (event.shiftKey ? 0.5 : 2), t12 = constrain(translate(scale2(t02, k1), p0, p1), extent3.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0)
      select_default(this).transition().duration(duration).call(schedule, t12, p0, event);
    else
      select_default(this).call(zoom.transform, t12, p0, event);
  }
